...
class Tru_Fetcher_Admin_Blocks_Resources_Listings extends Tru_Fetcher_Admin_Blocks_Resources_Base
{
    ...
    public array $config = [];
    private Tru_Fetcher_Api_Helpers_Setting $settingsHelper;

    public function __construct()
    {
        $this->keymapHelpers = new Tru_Fetcher_Api_Helpers_Keymaps();
        $this->settingsHelper = new Tru_Fetcher_Api_Helpers_Setting();
        $this->initConfig();
            ...
            'attributes' => [
                ...
                [
                    ...
                    'options' => function () {
                        ...
                        $customTemplates = $this->settingsHelper->getSetting('custom_templates');
                        if (!is_array($customTemplates) || !count($customTemplates)) {
                            return $options;
                        }
                        $options = [
                            ...$options,
                            ...
                            }, array_filter($customTemplates, function ($template) use($options) {
                                return !in_array($template['name'], array_map(function ($option) {
                                    return $option['value'];
                                }, $options));
                            }))
                        ];
                ...
                [
                    ...
                    'options' => function () {
                        ...
                        foreach ($customTemplates as $template) {
                            ...
                            $options = [
                                ...$options,
                                ...array_map(function ($style) use($template) {
                                    return [
                                        'value' => $style['name'],
                                        'label' => "{$style['name']} - ({$template['name']})",
                                    ];
                                }, $template['styles'])
                            ];
                        }
