...
use TruFetcher\Includes\DB\Repository\Relations\Tru_Fetcher_DB_Repository_Relations;
...
class Tru_Fetcher_DB_Repository_Base
{
    ...
    private Tru_Fetcher_DB_Repository_Relations $relations;
    ...
    public function __construct(Tru_Fetcher_DB_Model $model)
    {
        ...
        $this->relations = new Tru_Fetcher_DB_Repository_Relations($this);
        $this->model = $model;
        $this->initialise();
    }

    public function getRelations(): Tru_Fetcher_DB_Repository_Relations
    {
            ...
            switch ($whereData['compare']) {
                ...
                case Tru_Fetcher_DB_Model_Constants::WHERE_COMPARE_IN:
                    if (!is_array($whereData['value'])) {
                        $this->addError(
                            new \WP_Error(
                                'tru_fetcher_db_where_in_error',
                                'Where in value is not an array',
                                $whereData
                            )
                        );
                        break;
                    }
                    if (count($whereData['value']) === 0) {
                        $this->addError(
                            new \WP_Error(
                                'tru_fetcher_db_where_in_error',
                                'Where in value is empty',
                                $whereData
                            )
                        );
                        break;
    ...
    protected function buildQuery()
    {
        ...
        if (count($this->where) || count($this->whereGroups)) {
            ...
            if ($whereData && is_string($whereData) && strlen($whereData)) {
    ...
    private function buildSaveDataWhereConditionsArray(array $data)
    {
        ...
        foreach ($this->getWhereQueryConditions() as $index => $condition) {
            ...
            $whereConditions[] = [
                ...
                Tru_Fetcher_DB_Model_Constants::VALUE => $this->db->getDbValue($this->convertValueToType($value,
                    $condition))
            ];
        }
    ...
    private function buildSaveDataWhereConditionsStringArray(array $data)
    {
        ...
        foreach ($whereConditions as $index => $condition) {
            ...
            $value = $condition[Tru_Fetcher_DB_Model_Constants::VALUE];
    ...
    private function buildSaveDataWhereConditionsFetchArray(array $data)
    {
        ...
        $buildWhereStrings = [];
        foreach ($whereConditions as $index => $condition) {
            $buildWhereStrings[$condition[Tru_Fetcher_DB_Model_Constants::FIELD_KEY]] = $condition[Tru_Fetcher_DB_Model_Constants::VALUE];
        }
